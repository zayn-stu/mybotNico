const { getPandaCount, getLeaderboard, getStaffHidden, setStaffHidden } = require('./storage');
const { getVictoriesMap } = require('./victoryStorage');
const { startTimer, endTimer } = require('../../utils/perfMetrics');
const { getDisplayName } = require('./memberDisplayNames');
...
const DEGENERACY_GUILD_ID = process.env.DEGENERACY_GUILD_ID || '';
const DEGENERACY_STAFF_ROLE_ID = process.env.DEGENERACY_STAFF_ROLE_ID || '';
...
function isStaff(member) {
  if (!DEGENERACY_STAFF_ROLE_ID) return false;
  return member.roles.cache.has(DEGENERACY_STAFF_ROLE_ID);
}
...
const subcommands = {
  ...
  async staff(message, args) {
    // Only available for Degeneracy guild
    if (message.guild.id !== DEGENERACY_GUILD_ID) {
  ...
  async help(message) {
    const isDegeneracy = message.guild.id === DEGENERACY_GUILD_ID;
    let helpText =
      '**Panda Commands:**\n' +
